...
const props = withDefaults(defineProps<DsLineChartProps>(), {
  showArea: false,
  showDots: true,
  showGrid: true,
  showLegend: true,
...
function getColor(ds: DsLineChartDataset, idx: number): string {
  return ds.color ?? LINE_COLORS[idx % LINE_COLORS.length]
}
...
function areaPath(data: number[]): string {
  const n = data.length
  const base = yAt(minVal.value).toFixed(1)
  return `${linePath(data)} L ${xAt(n - 1).toFixed(1)} ${base} L ${xAt(0).toFixed(1)} ${base} Z`
}
    ...
    <div ref="wrapperRef" class="relative" @mousemove="onMouseMove" @mouseleave="onMouseLeave">
      ...
      >
        ...
        <template v-if="showArea">
          <path
            v-for="(ds, di) in datasets" :key="`area-${di}`"
            :d="areaPath(ds.data)"
            :fill="getColor(ds, di)"
            fill-opacity="0.12"
          />
        ...
        <template v-if="showDots">
          <template v-for="(ds, di) in datasets" :key="`dots-${di}`">
            <circle
              v-for="(val, vi) in ds.data" :key="vi"
              :cx="xAt(vi)" :cy="yAt(val)" r="4"
              :fill="getColor(ds, di)"
